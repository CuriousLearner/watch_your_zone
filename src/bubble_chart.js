...
function bubbleChart() {
  ...
  var fillColor = d3.scale.ordinal()
    ...
    .range(['#34F060', '#2CCDEF', '#348FFF', '#7AEB2F', '#FAC93B', '#F88248', '#E6F046', '#EE3838', '#F5593B']);
  ...
  var chart = function chart(selector, rawData) {
    ...
    bubbles = svg.selectAll('.bubble')
      .data(nodes, function (d) { return d[chart_name]; });
    ...
    bubbles.enter().append('circle')
      .classed('bubble', true)
      .attr('r', 0)
      .attr('fill', function (d) { return fillColor(d.id); })
      //.attr('stroke', function (d) { return d3.rgb(fillColor(d.id)).darker(); })
      .attr('stroke-width', 2)
  ...
  function showDetail(d) {
    ...
    d3.select(this).attr('fill', LightenDarkenColor(this.attributes.fill.value, 70));
    //console.log(d);
    var content = '<span class="name">Zone: </span><span class="value">' +
                  d.name +
                  '</span><br/>' +
  ...
  function hideDetail(d) {
    // reset outline
    d3.select(this)
      //.attr('stroke', d3.rgb(fillColor(d.id)).darker())
      .attr('fill', d3.rgb(fillColor(d.id)));
...
function scale_x_between_min_and_max(min, max, a, b, x) {
  return (((b - a) * (x - min)) / (max - min)) + a;
}

var chart_name = 'education';
var desc = document.getElementById('desc');
desc.innerHTML = "Percentage of women with 10+ years of education.";

var refresh_display = function() {
  var vis = document.getElementById("vis");
  vis.childNodes[0].outerHTML = "";
  delete vis.childNodes[0];
  d3.csv('data/data.csv', display);
};
